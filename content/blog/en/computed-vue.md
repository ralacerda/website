...
In my last post, I talked about how Vue batches template functions and `watch` [effects](/batch-effects-vue). However, there's another form of reactivity in Vue: computed properties, using `computed`. The `computed` function is used for derived state, applying operations over one or more `refs`.
